...
my_model = load_my_CNN_model('glasses_model.h5')
...
# Define filters
filters = ['glasses/red-glasses-png-3.png']
filterIndex = 0

def ratio_func_width(glasses_original_shape, glasses_width_shape, original_image_shape):
    percentege = (glasses_width_shape/glasses_original_shape)*100
    # print(percentege)
    width = (original_image_shape/100)*percentege
    # print(int(width))
    return int(width)

def ratio_func_hight(glasses_original_shape, glasses_hight_shape, original_image_shape):
    percentege = (glasses_hight_shape/glasses_original_shape)*100
    # print(percentege)
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        x_original, y_original = gray_face.shape # A Copy for future reference
        ...
        frame[y:y+h, x:x+w] = cv2.resize(face_resized_color, (x_original, y_original), interpolation = cv2.INTER_CUBIC)

        # Add KEYPOINTS to the frame2
        for keypoint in points:
            cv2.circle(face_resized_color2, keypoint, 1, (0,255,0), 1)

        frame2[y:y+h, x:x+w] = cv2.resize(face_resized_color2, (x_original, y_original), interpolation = cv2.INTER_CUBIC)



        
        sunglasses_test = cv2.imread(filters[0])    
        sunglasses_test = cv2.flip(sunglasses_test,0)


        row_test, column_test, alpha = frame3[y:y+h, x:x+w].shape
        width = ratio_func_width(96, sunglass_width+5, row_test)
        hight = ratio_func_hight(96, sunglass_height, column_test)
        ...
        for i in range(0, gw):
            for j in range(0, gh):
                # print(sunglasses_test[i,j])
                if sunglasses_test[i,j][1] !=0:
                    try:
                        color_face_copy[h-i-105, w-j-30] = sunglasses_test[i,j]
                    except:
                        pass

        frame3[y:y+h, x:x+w] = cv2.resize(color_face_copy, (x_original, y_original))
